...
import { ensurePlaceholders, makeFxTextures } from '../core/placeholders';
...
export class BootScene extends Phaser.Scene {
  ...
  create() {
    ...
    const jobs = IMAGE_KEYS.filter((k) => manifest.images[k]).map((k) => loadSvgTexture(this.textures, k, manifest.images[k], IMAGES[k].box));
    Promise.all(jobs)
