 ...
 * agent's gen_prep_e.py from the same outlines that draw veg-*-whole). All numbers are in the 672x504 whole-vegetable
 * file's units (the vegetables rest on the board at about y 475).
 */
export type VegName = 'tomato' | 'mushroom' | 'pepper' | 'onion';

export const VEG = {
  /** The veg-*-whole frame. */
  size: [672, 504] as const,
  /** The body's x span: cutting goes from right to left inside it (the pepper's stem is not cut). */
  span: { tomato: [125, 547], mushroom: [91, 581], pepper: [60, 588], onion: [31, 636] } as Record<VegName, readonly [number, number]>,
  /** veg-*-inside strips: 60 wide, as tall as the body at its tallest. */
  stripW: 60,
  stripH: { tomato: 378, mushroom: 406, pepper: 378, onion: 327 } as Record<VegName, number>,
