"""Targeted fixes to the cookie lines in voice-a-mom (2026-09-19). Everything else keeps make_vo.py's processing.
Reuses fix_vo.py / fix_vo_salad.py functions unchanged (measurement, approach-B cut, soft limiter, normalize, encode).

  python fix_vo_cookies.py measure  -> measures the 23 cookie lines (lead/trail extra vowel + speech recognizer) into work/vo/fix-cookies/measure.json
  python fix_vo_cookies.py build    -> renders candidates into work/vo/fix-cookies/ and measures them
  python fix_vo_cookies.py apply    -> backs up the originals to work/vo/pre-fix-cookies/, copies the chosen candidates into voice-a-mom/,
...
ROOT = F.ROOT
WORK = F.WORK = ROOT / "work/vo/fix-cookies"
PRE = ROOT / "work/vo/pre-fix-cookies"
LINES = ["vo-pick-cookies", "vo-flour", "vo-sugar", "vo-butter", "vo-egg", "vo-stir-dough", "vo-knead-cookies", "vo-roll-cookies",
         "vo-pick-cutter", "vo-stamp", "name-star", "name-heart", "name-circle", "name-flower", "vo-tray", "vo-temp-150",
         "vo-decorate-cookies", "vo-share-cookies", "vo-cookie-mom", "vo-cookie-pipa", "vo-cookie-yum", "vo-photo-cookies", "vo-finale-cookies"]
F.TAIL_CUT = set()
S.START_RUN = {"vo-egg": 0}  # B: start cut at the "k" of "Crack" (a short voiced "uh" before it, recognizer heard "the crack")
...
def build(specs):  # e.g. fix_vo_cookies.py build vo-decorate-cookies=L vo-egg=B vo-flour=T
    WORK.mkdir(parents=True, exist_ok=True)
    res = {}
    for name, kind in specs.items():
        ...
        if kind == "L":
            ...
            y, info["cut_at_s"] = S.cut(x, sr, name); z = F.normalize(y, sr)
        else:
