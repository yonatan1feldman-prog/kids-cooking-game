#!/bin/bash
# One screenshot at a time (images-b-smoothie), file:// only (no HTTP server): needs >= 2048 MB free RAM
...
# usage: tools/shot.sh <out.png> <page?query relative to images-b-smoothie/> [cssW cssH dpr]
# Also waits while ~/cooking/.browser-busy exists (another agent is using a browser): re-check every 30 s, up to 15 min.
...
BUSY="${BROWSER_BUSY:-$HOME/cooking/.browser-busy}"
for i in $(seq 1 31); do [ -e "$BUSY" ] || break; echo "waiting: .browser-busy exists"; sleep 30; done
if [ -e "$BUSY" ]; then echo "SKIP: .browser-busy still present after 15 min"; exit 4; fi
