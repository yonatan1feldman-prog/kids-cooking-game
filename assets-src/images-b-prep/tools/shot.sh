#!/bin/bash
# One screenshot at a time (images-b-prep): refuse unless >= 2048 MB RAM is free; one hidden Edge, closed afterwards.
# usage: tools/shot.sh <out.png> <page?query relative to cooking-game-assets/images-b-prep/> [cssW cssH dpr]
...
for i in 1 2 3 4 5 6; do F=$(free_mb); [ "$F" -ge 2048 ] && break; echo "waiting: only ${F} MB free"; sleep 10; done
...
"$EDGE" --headless=new --disable-gpu --hide-scrollbars --no-first-run --user-data-dir="$(cd "$PROFILE" && pwd -W)" \
  ...
  --screenshot="$ABS" "http://localhost:8765/images-b-prep/$Q" 2>/dev/null
